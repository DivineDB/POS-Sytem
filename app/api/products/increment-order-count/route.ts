...
export async function POST(request: Request) {
  try {
    ...
    const { error } = await supabase
      .rpc('increment_order_count', { product_id: productId })

    if (error) {
      console.error('[API /api/products/increment-order-count] RPC error:', error)
      return NextResponse.json({ error: error.message }, { status: 500 })
    }

    return NextResponse.json({ success: true })
  } catch (err: any) {
    console.error('[API /api/products/increment-order-count] Unexpected error:', err)
